...
/// The entry point of the NBT's asynchronous data transfer (ADT) mobile phone application.
/// The iOS application demonstrates how to leverage the asynchronous data transfer (ADT)
/// capabilities of the NBT device.
///
/// This ``InfineonNBTADTApp`` struct conforms to the `App` SwiftUI protocol and serves as the main
...
@main
struct InfineonNBTADTApp: App {
    /// The body of the ``InfineonNBTADTApp`` scene.
    ///
    ...
    var body: some Scene {
        WindowGroup {
            ZStack(alignment: .top) {
                SplashScreenView().environmentObject(mViewModel)
            }
        }
    }
}
